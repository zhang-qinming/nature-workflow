...
TASK_NAME="${TASK_NAME:-posterior_volcano}"
JOB_NAME="${JOB_NAME:-pvol}"

OUTPUT_ROOT="${OUTPUT_ROOT:-/gpfs/chencao/qinminzhang/workflow/catalog_lof/figure_all/outputs}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/posterior_volcano}"
...
for key in ("file_id_map", "posterior_dir", "gene_map", "geneset_dir", "spectra_path"):
...
config["workflows"]["figures"]["posterior_volcano"] = {
...
paper-pipeline run --config "${TEMP_CONFIG}" figures-posterior-volcano
