...
TASK_NAME="${TASK_NAME:-gene_level_qq}"
JOB_NAME="${JOB_NAME:-glqq}"

OUTPUT_ROOT="${OUTPUT_ROOT:-/gpfs/chencao/qinminzhang/workflow/catalog_lof/figure_all/outputs}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/gene_level_qq}"
...
QQ_Y_LIMIT="${QQ_Y_LIMIT:-10}"
QQ_RENDER_PLOT="${QQ_RENDER_PLOT:-0}"
...
python3 - "${LOF_ID}" "${PROJECT_ROOT}" "${BASE_CONFIG}" "${TEMP_CONFIG}" "${OUTPUT_ROOT}" "${OUTPUT_DIR}" "${FILE_ID_MAP}" "${LIMMA_PATH}" "${SHET_PATH}" "${QQ_Y_LIMIT}" "${QQ_RENDER_PLOT}" <<'PYEOF'
import sys
...
qq_y_limit = float(sys.argv[10])
qq_render_plot = str(sys.argv[11]).lower() in {"1", "true", "yes"}
...
config["workflows"]["figures"]["gene_level_qq"] = {
    "inputs": {
...
paper-pipeline run --config "${TEMP_CONFIG}" figures-gene-level-qq
