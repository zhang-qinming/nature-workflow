...
STANDARDIZE_SCRIPT="${STANDARDIZE_SCRIPT:-${PROJECT_ROOT}/scripts/standardize_kolf_for_pipeline.py}"
...
K_VALUES="${K_VALUES:-30 60 120 180 240 300 360}"
...
STANDARDIZE_MEM="${STANDARDIZE_MEM:-900G}"
...
KSELECT_PREP_MEM="${KSELECT_PREP_MEM:-900G}"
...
KSELECT_FACTORIZE_MEM="${KSELECT_FACTORIZE_MEM:-120G}"
...
KSELECT_POST_MEM="${KSELECT_POST_MEM:-160G}"
KSELECT_POST_CPUS="${KSELECT_POST_CPUS:-24}"
KSELECT_POST_PARTITION="${KSELECT_POST_PARTITION:-cu,privority,batch01}"
KSELECT_POST_TIME="${KSELECT_POST_TIME:-2-00:00:00}"

CONSENSUS_MEM="${CONSENSUS_MEM:-160G}"
CONSENSUS_CPUS="${CONSENSUS_CPUS:-24}"
...
write_limma_note() {
    cat > "${BATCH_ROOT}/LIMMA_NOT_GENERATED.txt" <<EOF
KOLF limma jobs are intentionally not generated here.

The existing gene-level limma workflow densifies the full h5ad matrix to CSV
and repeatedly reads large control matrices. For KOLF
(${KOLF_INPUT_H5AD}), that is not a safe default. Use a sparse/chunked or
pseudobulk KOLF-specific limma implementation instead.
EOF
}
...
Limma note:
  ${BATCH_ROOT}/LIMMA_NOT_GENERATED.txt
EOF
