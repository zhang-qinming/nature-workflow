...
TRAIT=if (length(args) >= 5) as.character(args[5]) else "MCH"
K=if (length(args) >= 6) as.numeric(args[6]) else 60
mch_posterior_path <- if (length(args) >= 7) as.character(args[7]) else "data/LoF/GeneBayes_posterior/Backman_2021_86.per_gene_estimates.tsv"
rdw_posterior_path <- if (length(args) >= 8) as.character(args[8]) else "data/LoF/GeneBayes_posterior/Backman_2021_88.per_gene_estimates.tsv"
irf_posterior_path <- if (length(args) >= 9) as.character(args[9]) else "data/LoF/GeneBayes_posterior/Backman_2021_106.per_gene_estimates.tsv"
...
df<-df[,c("post_mean", paste0("P", seq_len(K)), "shet")]
for(i in 2:ncol(df)){
