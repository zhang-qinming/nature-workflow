...
summary<-data.frame(SEED="true", meangamma=mean(as.numeric(df_pro[[screen_column]]), na.rm=T))
...
hoge<-data.frame(SEED=I, meangamma=mean(as.numeric(df_ctrl[is.element(df_ctrl$gene, genes), screen_column]), na.rm=T))
