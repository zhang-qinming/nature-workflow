...
LOF_rawZ<-read.csv("data/LoF/raw_burden/Backman_2021_86_M1_001.summary_statistics.csv", header=T, stringsAsFactor=F)
...
ensg<-corresp[is.element(corresp[,2], "HBA1"), 1]
...
write.table(summary, paste0("MCH_GWAS_HBA1regulator_enrichment.txt"), row.names=F, sep="\t", quote=F)
...
write.table(summary, paste0("MCH_LoF_HBA1regulator_enrichment.txt"), row.names=F, sep="\t", quote=F)

##make figure

LOF_enrich<-read.table("MCH_LoF_HBA1regulator_enrichment.txt", header=T, stringsAsFactor=F)
GWAS_enrich<-read.table("MCH_GWAS_HBA1regulator_enrichment.txt", header=T, stringsAsFactor=F)
