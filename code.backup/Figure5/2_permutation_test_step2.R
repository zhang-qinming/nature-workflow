...
data<-read.table(paste0("perutation_test/P", TOP_N, "/", FILE), header=T, stringsAsFactor=F)
...
data<-read.table("permutation_test/P200/MCH_program5_regulator3_LOF0.1.txt", header=T, stringsAsFactor=F)
...
ggsave("P200_MCH_program5_regulator3_thresh0.1_hist.pdf")

##

data_f<-read.table("permutation_test/P200/MCH_program5_regulator3_LOF0.1.txt", header=T, stringsAsFactor=F)
...
ggsave(plot=g, "MCH_concordant_discordant_gene_num_scatter.pdf", height=8, width=8)
