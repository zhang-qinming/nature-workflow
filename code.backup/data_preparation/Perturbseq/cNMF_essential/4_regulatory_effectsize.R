...
library(data.table)
count<-fread(paste0("data/Perturbseq/cNMF/", CELL, "/test1/test1.usages.k_60.dt_0_4.consensus.txt"), header=T, data.table=F)
...
write.table(summary, paste0("data/Perturbseq/cNMF_regulation/", CELL, "/K60_program", colnames(count)[i],  "_perturb_effects.txt"), row.names=F, sep="\t", quote=F, append=F)
